          ...
          if(event.origin == location.protocol +'//'+ location.host) {
            if(event.data.substring(0, 5) == 'conn:') {
              ...
              for(var i=0; i<categories.length; i++) {
                clients[categories[i]] = createClient(data.storageInfo, categories[i], data.bearerToken);
              }
              cb(clients, data.storageInfo, categories, data.bearerToken);
            }
          }
        }, false);
      },
      sync = function(userAddress, categories) {
        connect(userAddress, categories, startSync);
      },
      startSync = function(clients, storageInfo, categories, bearerToken) {
